"""
graph/workflow.py — Graphe LangGraph avec 3 routes : quran / hadith / conversation.
"""
...
from agents.domain_agent import run_quran_agent, run_hadith_agent, run_conversation_agent
...
def node_conversation(state: AgentState) -> dict:
    answer = run_conversation_agent(state["question"])
    return {"messages": [AIMessage(content=answer)]}
...
def build_workflow():
    ...
    graph.add_node("conversation_agent",  node_conversation)
